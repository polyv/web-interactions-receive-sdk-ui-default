<template>
  <div class="plv-iar-questionnaire-answer">
      ...
      <div class="plv-iar-questionnaire-answer__options">
        ...
        <template v-if="answer.type === 'Q'">
          <div class="plv-iar-questionnaire-answer__option__q">
            <p>{{ answer.answer }}</p>
            <span><strong>{{ answer.total }}</strong>人</span>
          </div>
        </template>
        <!-- 五星评分 -->
        <div v-else-if="answer.type === 'X'" class="plv-iar-questionnaire-answer__option__x">
          <score-item
            type="X"
            scene="display"
        ...
        <!-- 10分评分 -->
        <div v-else-if="answer.type === 'S'" class="plv-iar-questionnaire-answer__option__s">
          <score-item
            type="S"
            scene="display"
            :amount="starOrScoreAmount"
        ...
        <template v-else>
          ...
          >
            <!-- 单选 -->
            <div v-if="answer.type === 'R'" class="plv-iar-questionnaire-answer-type">
            ...
            <!-- 多选 -->
            <div v-else-if="answer.type === 'C'" class="plv-iar-questionnaire-answer-type">
              <span class="plv-iar-questionnaire-answer__option__text">{{ optionChar(optionIndex) }} . {{ optionItem.value }}</span>
              <i class="plv-iar-questionnaire-answer__option--icon"></i>
              <div class="plv-iar-questionnaire-answer__option__mobile--border"></div>
            </div>
            ...
            <!-- 判断 -->
            <div v-else-if="answer.type === 'J'" class="plv-iar-questionnaire-answer-type">
              <span class="plv-iar-questionnaire-answer__option__text">{{ optionItem.value }}</span>
              <i class="plv-iar-questionnaire-answer__option--icon"></i>
              <div class="plv-iar-questionnaire-answer__option__mobile--border"></div>
            </div>
            <span class="plv-iar-questionnaire-answer__option--stats"><strong>{{ optionItem.selectUserNum }}</strong>人</span>
            <span class="plv-iar-questionnaire-answer__option--stats">（{{ optionItem.ratio }}）</span>
          </div>
        </template>
      </div>
    </div>
...
.plv-iar-questionnaire-answer__option {
  ...
  display: flex;
  align-items: center;
  .plv-iar-questionnaire-answer-type {
    flex: 1;
  }
  > div {
    position: relative;
...
.plv-iar-questionnaire-answer__option__x,
.plv-iar-questionnaire-answer__option__s {
  display: flex;
  align-items: center;
  font-size: 14px;
  strong {
